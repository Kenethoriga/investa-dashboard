...
class InvestmentAccountViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        if self.action == 'retrieve' and self.request.user.accounts.filter(account_type='view_only').exists():
            self.permission_classes = [IsViewOnly]
        elif self.action in ['create', 'update', 'delete'] and self.request.user.accounts.filter(account_type='full_access').exists():
            self.permission_classes = [IsFullAccess]
        elif self.action == 'create' and self.request.user.accounts.filter(account_type='post_only').exists():
